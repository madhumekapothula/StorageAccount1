...
namespace StorageAccount.Repository
{
    ...
    {
        public static string connectionString="DefaultEndpointsProtocol=https;AccountName=madhustorageaccount530;AccountKey=xqZmAgBKTuLNImWQyKxRRA28mNarazbNOy1CzuNlyOwPARyTkcedgnwAbwZMSE+osECBstlhdKrp+AStcZLDcg==;EndpointSuffix=core.windows.net";
        ...
        {
            string path=@"C:\Users\vmadmin\Desktop\IotDemo\StorageAccount\Downloads\"+fileName;
            shareServiceClient=new ShareServiceClient(connectionString);
            var serviceClient=shareServiceClient.GetShareClient(fileShareName);
            var dir=serviceClient.GetDirectoryClient(directoryName);
            var file=dir.GetFileClient(fileName);
